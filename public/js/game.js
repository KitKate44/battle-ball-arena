...
let _bmGain = null;
let _bmScheduler = null;
let _bmClock = 0;
let _bmBeat = 0;

const _BPM  = 138;
const _BEAT = 60 / _BPM;   // quarter note ≈ 0.435s
const _STEP = _BEAT / 2;   // eighth note  ≈ 0.217s

// A-minor pentatonic bass line (0 = rest), A1 C2 E1 G1
const _BASS_PAT = [55, 0, 55, 65.41, 55, 0, 49, 41.2];
// Arpeggio melody on off-steps, A3 C4 E4 C4
const _ARP_PAT  = [0, 220, 0, 261.63, 0, 329.63, 0, 261.63];

function startBattleMusic() {
  if (!audioCtx || _bmGain) return;
  _bmGain = audioCtx.createGain();
  _bmGain.gain.setValueAtTime(0, audioCtx.currentTime);
  _bmGain.gain.linearRampToValueAtTime(0.3, audioCtx.currentTime + 1.8);
  _bmGain.connect(audioCtx.destination);
  _bmClock = audioCtx.currentTime + 0.05;
  _bmBeat  = 0;
  _schedBattle();
  _bmScheduler = setInterval(_schedBattle, 80);
}

function stopBattleMusic() {
  if (!_bmGain) return;
  clearInterval(_bmScheduler);
  _bmScheduler = null;
  const g = _bmGain;
  _bmGain = null;
...
function _schedBattle() {
  if (!_bmGain) return;
  const horizon = audioCtx.currentTime + 0.25;
  while (_bmClock < horizon) {
    _beatEvent(_bmClock, _bmBeat);
    _bmBeat  = (_bmBeat + 1) % 8;
    _bmClock += _STEP;
  }
}

function _beatEvent(t, beat) {
  if (beat === 0 || beat === 4)           _bmKick(t);
  if (beat === 2 || beat === 6)           _bmSnare(t);
  _bmHihat(t, beat % 2 === 0 ? 0.3 : 0.15);
  if (_BASS_PAT[beat])  _bmBass(t, _BASS_PAT[beat]);
  if (_ARP_PAT[beat])   _bmArp(t, _ARP_PAT[beat]);
}
...
function _bmKick(t) {
  ...
  osc.connect(gain); gain.connect(_bmGain);
  osc.start(t); osc.stop(t + 0.3);
}

function _bmSnare(t) {
  const size = Math.ceil(audioCtx.sampleRate * 0.12);
  ...
  src.connect(filter); filter.connect(gain); gain.connect(_bmGain);
  src.start(t); src.stop(t + 0.14);
}

function _bmHihat(t, vol) {
  const size = Math.ceil(audioCtx.sampleRate * 0.03);
  const buf  = audioCtx.createBuffer(1, size, audioCtx.sampleRate);
  const data = buf.getChannelData(0);
  for (let i = 0; i < size; i++) data[i] = Math.random() * 2 - 1;
  const src = audioCtx.createBufferSource();
...
function _bmBass(t, freq) {
  const osc = audioCtx.createOscillator(), filter = audioCtx.createBiquadFilter(), gain = audioCtx.createGain();
  osc.type = 'sawtooth'; osc.frequency.value = freq;
  filter.type = 'lowpass'; filter.frequency.value = 380; filter.Q.value = 2.5;
  gain.gain.setValueAtTime(0.45, t);
  gain.gain.exponentialRampToValueAtTime(0.001, t + _STEP * 0.88);
  osc.connect(filter); filter.connect(gain); gain.connect(_bmGain);
  osc.start(t); osc.stop(t + _STEP * 0.9);
}

function _bmArp(t, freq) {
  const osc = audioCtx.createOscillator(), gain = audioCtx.createGain();
  osc.type = 'square'; osc.frequency.value = freq;
  gain.gain.setValueAtTime(0.06, t);
  gain.gain.exponentialRampToValueAtTime(0.001, t + _STEP * 0.65);
  osc.connect(gain); gain.connect(_bmGain);
  osc.start(t); osc.stop(t + _STEP * 0.7);
}
